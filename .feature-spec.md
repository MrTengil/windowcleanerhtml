...
**Fix: your own suggestion — average over a short rolling window of
recent per-frame deltas, not the single latest one.** New stateful
`SweepSmoother` class in `src/interactions/SweepSmoother.js`
(alongside `RevealTracker`, the same kind of small
interaction-tracking state, grown with strict TDD — the windowing/
eviction behavior is a genuine unknown worth discovering test-by-test):
- `addSample({ dx, dy })` pushes a frame's delta, evicting the oldest
  once a fixed `windowSize` is exceeded
- `averageDelta()` returns the mean `{ dx, dy }` of whatever's
  currently in the window
- `reset()` clears the window — called on `pointerdown`, so a new
  swipe never inherits a stale direction left over from the previous
  one

`computeSweepRotation({ dx, dy })` itself is untouched (already
correct, already tested) — `rotateToolTowardSweep()` now calls
`this.sweepSmoother.addSample(...)` with the raw per-frame delta, then
passes `this.sweepSmoother.averageDelta()` into `computeSweepRotation`
instead of the raw delta directly. `this.sweepSmoother = new
SweepSmoother({ windowSize: SWEEP_SMOOTHING_WINDOW })` is constructed
once alongside the existing `buildToolIcon()`; `setupSwipeInput()`
gains a `pointerdown` listener (there wasn't one before — only
`pointermove`/`pointerup`) that resets it.

**`SWEEP_SMOOTHING_WINDOW` starting value: 5 frames** — a guess, not
derived; per the same "I'll check it in a browser and adjust" caveat
as Revision 14/15's constant-tuning. Too small and the flicker persists,
too large and the icon visibly lags behind quick direction changes —
I'll watch for both when verifying.

**Verification:** unit tests cover the windowing/averaging/reset logic
in isolation (real design decisions worth pinning down: does a single
sample echo back unchanged, does eviction actually drop the oldest,
does reset actually clear). The Phaser wiring is glue, verified
visually like every other scene-facing change in this file — no
meaningful way to unit-test "does dragging feel less jittery."

Out of scope: making the window size configurable per tool (only
