...
## Revision 43 progress

Implemented the planned snap-to-center fix, plus a second, more fundamental
bug found while verifying it — direct implementation, no new pure logic, one
commit, 68 tests still passing, no lint issues.

- [x] New `DIRT_SPOT_SNAP_RADIUS = 90` and `findSprayTargetPosition(x, y)`:
      resolves to an uncleared dirt spot's exact `(x, y)` when one is within
      that radius, otherwise the raw press position unchanged. Wired into
      both `startSprayHold()` and `tickSprayHold()` before
      `findOrCreateSprayDecal()`.
- [x] **Found and fixed a second bug, not in the plan, that was fully
      blocking hand-prints cleaning independent of the snap fix:**
      `eraseAlongPath()`'s per-step loop (added in Revision 42) calls
      `wipeDirtSpotsNear(x, y)` and `eraseSprayDecalsNear(x, y)` back to back
      at the *same* point on every step. The first successful wipe step both
      registers a tap **and** is close enough to erase the very decal that
      just satisfied its own spray precondition — so the decal was gone
      before the remaining `hitsToClean - 1` taps could ever register,
      permanently stalling a hand-print at 1 tap no matter how much more you
      wiped (bird-poop's hold has the same shape of problem, just never
      surfaced since it doesn't call `eraseAlongPath` at all). Confirmed via
      headless verification: a snapped, correctly-registering spray decal
      (`sprayStageAtSpot: 1`) still failed to clear an actual hand-print
      until this fix landed.

      Fix: a new `decalGuardsUnclearedSpot(decal)` — true when any *uncleared*
      dirt spot sits within `SPRAY_SPOT_RADIUS` of the decal — makes
      `eraseSprayDecalsNear()` skip decals still serving as an active
      precondition. `clearDirtSpot()` (shared by both hand-prints and
      bird-poop) now explicitly calls `eraseSprayDecalsNear(spot.x, spot.y)`
      right after marking the spot cleared, so the residue still gets cleaned
      up promptly — just once it's actually safe to.

**Verified via headless Chrome:** spraying 75-80px away from a spot's own
center (well outside the old `SPRAY_SPOT_RADIUS`/`DIRT_SPOT_HIT_RADIUS`, but
inside the new snap radius) produces a decal whose `(x, y)` exactly equals
the spot's own — not the press position; `sprayStageAt(spot)` immediately
reports the built-up stage; a full wipe now actually clears the hand-print
(previously stalled after 1 of 6 taps); bird-poop reaches max spray stage (4)
the same way; spraying away from any dirt spot still creates/grows a decal at
the press position, unaffected; directly driving 6 wipes against a pre-sprayed
spot confirms the spot clears and its decal count drops from 1 to 0. No
console errors in any run.

No units remaining from this session's agreed scope.

# Revision 44
 - Make the spray larger
 - Make Generate more bubbles. Spread them out more. Vary in size
 - Make the spray pattern more visible.

## Revision 44 plan

Continues on `main`.

**1. Make the spray decal larger.** `SPRAY_DECAL_DISPLAY_SIZE` 120 → 170
(starting guess, tuned visually). Following the exact lesson from Revisions
42/43 (enlarging a sprite without its associated radius created a real
visual-vs-logical mismatch bug), also bumping `SPRAY_SPOT_RADIUS` 60 → 85 to
match the new visual radius — it governs "is a new press near the same
existing decal" and the spot/erase/guard distance checks, so it should track
the decal's actual visual footprint.

**2. Bubbles — three sub-fixes:**
- **Vary in size (a real bug fix):** `BUBBLES` already has 5 differently-
  named-and-sized assets (tiny→xlarge), but `spawnHoldBubble()` forces every
  one of them to the same flat `BUBBLE_DISPLAY_SIZE = 40` via
  `setDisplaySize()` — so the variety in the art never shows up. Fix: give
  each `BUBBLES` entry its own `displaySize` (22/32/42/54/68 for
  tiny→xlarge) and use that instead of the flat constant.
- **More bubbles:** spawn a few per tick instead of one — new
  `BUBBLE_SPAWN_COUNT_PER_TICK = 3`, each with its own independent
  jitter/random asset pick.
- **Spread them out more:** `BUBBLE_JITTER` 20 → 45 (starting guess).

**3. Make the spray pattern more visible** (per your chosen approach): the
low visibility is baked into the 4 `spray_pattern_*.svg` assets themselves —
every path is a pale near-white blue at opacity 0.12–0.8, and Phaser's own
image alpha is already 1, so there's no runtime dial. A small one-off script
(scratchpad only, not committed) scales every `opacity="…"` attribute across
all 4 files by a fixed multiplier (starting guess ×1.6, capped at 1.0),
preserving the existing wet/droplet look while making it read more clearly
against the window pane and dirt mask. The modified SVGs themselves are what
gets committed.

**No new pure logic** — constant tuning, a data-driven bubble-size fix, and
an asset transform, same category as most of this spec. Verified via
headless Chrome (decal/bubble sizes and counts read back from scene state)
plus a visual screenshot check for the SVG opacity change, since that one's
inherently a "does it look right" call.

## Revision 44 progress

Implemented all 3 items exactly as planned, no deviations — 68 tests still
passing, no lint issues, one commit per item:

- [x] `SPRAY_DECAL_DISPLAY_SIZE` 120 → 170; `SPRAY_SPOT_RADIUS` 60 → 85 to
      match.
- [x] `BUBBLES` entries each gained a `displaySize` (22/32/42/54/68 for
      tiny→xlarge), used in `spawnHoldBubble()` instead of the old flat
      `BUBBLE_DISPLAY_SIZE` (removed); new `spawnHoldBubbles()` calls it
      `BUBBLE_SPAWN_COUNT_PER_TICK = 3` times per tick; `BUBBLE_JITTER`
      20 → 45.
- [x] One-off scratchpad script scaled every `opacity="…"` value across all
      4 `spray_pattern_*.svg` files by ×1.6 (capped at 1.0) — 34/60/90/124
      values touched respectively (small→xlarge, more paths at the denser
      stages). The script itself was not committed, only its output (the
      modified SVGs).

**Verified via headless Chrome:** a full-hold spray decal reports
`displayWidth: 170`; a burst of bubbles during a bird-poop hold shows 9
images (3 ticks × 3 each) across multiple distinct display widths (not all
identical, confirming the size-vary fix actually renders); a screenshot
confirms the spray pattern now reads as a clearly visible white/pale-blue
splash against the dirt mask, rather than the near-invisible translucency
from before. No console errors.

No units remaining from this session's agreed scope.


# Revision 45
 - Put a circular progress bar around the bird poop which appears only when you press to hold and remove it. Make sure it's twice the size of the sponge

## Revision 45 plan

Continues on `main`.

Mirrors the existing screw-unscrewing progress ring (`buildScrew()`'s
`progressGraphics`, `updateScrewVisuals()`, hidden via `.setVisible(false)`
in `completeScrew()`) for bird-poop.

**1. Build the ring.** In `buildDirtSpots()`, bird-poop spots
(`interactionType === "hold"`) gain a `progressGraphics` — a
`this.add.graphics()` positioned at the spot's own container-local `(x, y)`,
added to the container alongside the sprite, initially invisible.

**2. Show/update it only while actively holding.** `startPoopHold()` makes it
visible once a cleanable spot is found; `tickPoopHold()` redraws the arc each
tick from `spot.progress.progressFraction()` (0→1), same
`arc(0, 0, radius, -π/2, -π/2 - fraction·2π, true)` shape as
`updateScrewVisuals()`. `stopPoopHold()` hides it again — consistent with
`HoldProgress.release()` already resetting progress to 0 on release
(Revision 41), so there's never stale partial progress to show once you let
go.

**3. Sizing: "twice the size of the sponge."** New
`POOP_PROGRESS_RADIUS = DRAGGING_TOOL_ICON_SIZE` (100) — the ring's radius
equals the dragging icon's own diameter, making the ring's diameter exactly
double the sponge icon's. New `POOP_PROGRESS_STROKE`/`POOP_PROGRESS_COLOR`
(starting with the same 6px/green `0x4caf50` as the screw ring for visual-
language consistency, kept as its own independent constant rather than
reusing the screw's directly, matching how board/tape already keep fully
separate constant sets even where values coincide).

**4. Cleanup.** `clearDirtSpot()` also destroys `spot.progressGraphics`
(only present on hold-type spots) alongside the existing sprite destruction,
so no orphaned Graphics objects linger once a spot is cleared.

**No new pure logic** — reuses `HoldProgress.progressFraction()`, already
tested; the rest is scene-glue mirroring an existing, proven pattern (the
screw ring). Verified via headless Chrome: ring is invisible before/after a
hold, visible and growing during one, hidden immediately on release (matching
the reset-to-zero progress), sized at the expected radius.

## Revision 45 progress

Implemented exactly as planned, no deviations — 68 tests still passing, no
lint issues, one commit.

- [x] `buildDirtSpots()`: hold-type spots (bird-poop) gain a `progressGraphics`
      (`this.add.graphics()`, positioned at the spot's container-local
      `(x, y)`, added to the container, initially invisible); wipe-type spots
      (hand-prints) get `null` — no ring, since they're a hit-counter, not a
      continuous hold.
- [x] New `POOP_PROGRESS_RADIUS = DRAGGING_TOOL_ICON_SIZE` (100, so the
      ring's diameter is exactly double the sponge icon's), plus its own
      `POOP_PROGRESS_STROKE`/`COLOR` (6px / green `0x4caf50`, matching the
      screw ring's values but kept independent).
- [x] `startPoopHold()` shows the ring and draws it at fraction 0;
      `tickPoopHold()` redraws it every tick via new
      `updatePoopProgressVisuals(spot)` (identical arc-drawing shape to
      `updateScrewVisuals()`); `stopPoopHold()` hides it before nulling
      `activePoopSpot` (order matters — has to read the still-valid
      reference); `clearDirtSpot()` destroys it alongside the sprite.

**Verified via headless Chrome:** ring is invisible on a fresh spot; visible
with `progressFraction() ≈ 0.37` partway through a real hold; hidden again
with fraction reset to 0 immediately after releasing early (matching
`HoldProgress.release()`'s existing zero-reset); no longer in the segment
container's display list once the spot is fully cleared. No console errors.

No units remaining from this session's agreed scope.

# Revision 46
 - Bird poop progression bar must be much smother. Now its choppy

## Revision 46 plan

Continues on `main`.

**Root cause.** The progress ring (and the underlying hold-time itself) only
advances inside `tickPoopHold()`, driven by a
`this.time.addEvent({ delay: POOP_HOLD_TICK_INTERVAL /* 100ms */, loop: true })`
timer — 10 updates/second. That's not just a rendering issue:
`HoldProgress.elapsedMs` itself only changes when `trackTime()` is called, so
the underlying data advances in visible ~100ms steps, not smoothly.
Redrawing more often wouldn't help — the value being drawn is genuinely
staircased.

**Fix: drive the hold from the scene's own `update(time, delta)` loop
instead of a fixed-interval timer.** `update()` already runs every rendered
frame (~60fps) with an accurate `delta`. Moving the "still holding the spot?"
check, `progress.trackTime(delta)`, and the ring redraw into a new
`updatePoopHold(delta)` called from `update()` makes the ring's own progress
advance in ~16ms steps instead of ~100ms ones — genuinely smooth, not just
redrawn more often.

**Bubbles must stay throttled, though** — spawning 3 per frame at 60fps
would be an explosion, not "more bubbles." A small `poopBubbleElapsed`
accumulator (incremented by `delta` each frame) keeps the existing burst
cadence: once it crosses the old 100ms interval (renamed
`POOP_BUBBLE_INTERVAL`, since that's now its only remaining purpose), it
resets and spawns a burst, same rate as today.

**This also simplifies the code**, since a separate Phaser timer object is no
longer needed at all: `startPoopHold()` no longer creates
`this.poopHoldTimer`; `stopPoopHold()` no longer removes one — the hold's
"active" state is just `this.activePoopSpot !== null`, checked every frame.

**No new pure logic** — `HoldProgress` itself needs zero changes (it's
already a generic "however much time you hand `trackTime()`" accumulator);
this is purely a scene-driver change. Verified via headless Chrome: sample
`progress.progressFraction()` across several consecutive frames during a
hold and confirm the increments are small/frame-sized rather than jumping in
~100ms chunks; confirm the ring still shows/hides/resets exactly as before;
confirm bubble count over a fixed hold duration is unchanged from today's
rate (no bubble-flood regression).

## Revision 46 progress

Implemented exactly as planned, no deviations — 68 tests still passing, no
lint issues, one commit.

- [x] `POOP_HOLD_TICK_INTERVAL` renamed `POOP_BUBBLE_INTERVAL` (its only
      remaining purpose). `startPoopHold()` no longer creates a
      `this.time.addEvent` timer — just resets a new `poopBubbleElapsed`
      accumulator, shows the ring, starts the vigor animation.
- [x] `tickPoopHold()` replaced by `updatePoopHold(delta)`, called from
      `update(time, delta)` every rendered frame: same "still holding?"
      guard as before, but `progress.trackTime(delta)` and the ring redraw
      now happen at frame rate; bubble bursts stay throttled via
      `poopBubbleElapsed += delta` crossing `POOP_BUBBLE_INTERVAL`.
- [x] `stopPoopHold()` simplified — no timer to remove, just hides the ring,
      releases progress, clears `activePoopSpot`, stops the vigor animation.

**Verified via headless Chrome:** recorded `progressFraction()` on every
`requestAnimationFrame` during a real 600ms hold — 37 samples, all 37
distinct, each consecutive step ≈0.00555 (matching 1 frame ⁄ 3000ms target),
with a max single-frame jump of the same ≈0.00555 — confirms genuinely smooth
per-frame progress, not the old ~0.033 (100ms) staircase. Bubble count over
one ~100ms hold window increased by exactly 3 (unchanged from the existing
`BUBBLE_SPAWN_COUNT_PER_TICK`), confirming no bubble-flood regression from
moving to a per-frame driver. No console errors.

No units remaining from this session's agreed scope.

# Revision 47
 - When I press and hold on the bird poop the sponge is not showing. I have to move the cursor before it shows. When I do that whatever is in the background gets cleaned. I dont want that. I just want the bird poop to be removed

## Revision 47 plan

Continues on `main`. Two distinct issues, both traced to root causes.

**1. Tool icon doesn't show until the cursor moves.** `handlePointerDown()`
never touches `this.toolIcon` at all — it's only ever shown/positioned inside
`handlePointerMove()`, which only fires on real `pointermove` events. A
stationary press-and-hold never generates one, so the icon stays invisible
until the first tiny jiggle. Fix: extract the show/hide/position logic
already in `handlePointerMove()` into a shared `updateToolIcon(pointer)`,
called from both `handlePointerDown()` (so it appears immediately on press)
and `handlePointerMove()` (unchanged continuous behavior).

**2. Moving the cursor while holding on bird-poop cleans the background
dust.** This is exactly why fixing problem 1 alone isn't enough: the fix
makes the icon appear without moving, but the user's actual complaint is
that any movement during a poop-hold — even the tiny jiggle needed to reveal
the icon today — runs `eraseAlongPath()` too, since sponge's
`canCleanWindow` isn't `false` (it's deliberately a general dust-erasing
tool, per Revision 41). While actively holding on a bird-poop spot, that's
not what the player wants. Fix: `handlePointerMove()` skips the generic
erase (`eraseAlongPath`/`updateRevealProgress`) whenever
`this.activePoopSpot` is set — it's only ever truthy while sponge is mid-hold
on a spot, so this doesn't affect sponge's normal dust-wiping or hand-prints'
wipe-to-clean at all.

**No new pure logic** — a small extraction plus one new guard, same
category as most of this spec. Verified via headless Chrome: icon becomes
visible immediately on a stationary `pointerdown` (no move needed) for a
normal tool; during an active bird-poop hold, jiggling the cursor around the
spot does not change the dirt mask / reveal progress at all, while the hold
itself still completes normally; sponge's ordinary dust-wiping (no active
poop hold) is unaffected.

## Revision 47 progress

Implemented exactly as planned, no deviations — 68 tests still passing, no
lint issues, one commit.

- [x] New `updateToolIcon(pointer)` — the show/hide/position logic
      extracted verbatim from `handlePointerMove()`'s top; called from both
      `handlePointerDown()` (new) and `handlePointerMove()` (unchanged call
      site, just relocated).
- [x] `handlePointerMove()`'s erase guard extended:
      `if (this.equippedTool.canCleanWindow === false || this.activePoopSpot) return;`
      — skips `eraseAlongPath`/`updateRevealProgress` while a bird-poop hold
      is active, regardless of how the cursor moves during it.

**Verified via headless Chrome:** a bare `mousedown` (no `mousemove` at all)
makes `toolIcon.visible` true immediately; spraying a bird-poop spot to max,
then holding sponge on it while jiggling the cursor in three directions
around the spot, leaves `revealTracker.revealedFraction()` at exactly `0`
throughout and after the hold, while `progress.progressFraction()` still
climbed normally (≈0.13 at 400ms) and the spot fully cleared on completion;
a separate, unrelated sponge drag (no active hold) still raised
`revealedFraction()` to ≈0.20 as expected — confirming ordinary dust-wiping
is unaffected. No console errors.

No units remaining from this session's agreed scope.

# Revision 48
 - Lets work on the spray pattern. I want to try something new.
 - Instead of change the spray pattern sprite lets have it build up. I have put some in spray-patterns/build-up. Use them (spray_layer_n) instead of the spray_patterns.. Also do randomized rotation on the layers when we add them.

## Revision 48 plan

Continues on `main`.

Investigated the new assets: 8 files in `assets/images/spray-patterns/build-up/`
(`spray_layer_1.svg`…`spray_layer_8.svg`), each with its own doc comment
confirming the intended mechanic exactly: "additive: stack layers 1..N in
order, on the SAME spot, to build up progressively wetter residue. Do not
resize/reposition between layers."

**Current behavior vs. new:** `setSprayDecalStage()` currently *replaces* the
decal's single image each time the stage advances (destroy old, create new
from `spray_pattern_N`). The new design keeps every layer's image alive,
stacking them.

**1. Config.** `SPRAY_PATTERNS` (kept as-is structurally, just repointed)
grows from 4 entries to 8, pointing at the new `build-up/spray_layer_N.svg`
files with new texture keys (`spray-layer-1`…`spray-layer-8`, replacing
`spray-pattern-1..4`). `BootScene` loads from the new `build-up/` subpath.
`SPRAY_MAX_STAGE` 4 → 8 to match the asset count.

**2. Additive stacking.** Each spray decal's `image: null` becomes
`layerImages: []`. `setSprayDecalStage(decal, stage)` no longer
destroys-and-replaces — it loops from `decal.stage + 1` to the new `stage`
and calls a new `addSprayLayer(decal, layer)` for each one (robust against
ever jumping more than one stage in a tick), which creates and keeps a new
image at the decal's position (same `SPRAY_DECAL_DISPLAY_SIZE`, per the
assets' own "don't resize/reposition" instruction) and pushes it onto
`layerImages`. Destruction sites (`eraseSprayDecalsNear`, `clearDirtSpot`'s
cleanup) destroy every image in `layerImages` instead of a single `image`.

**3. Randomized rotation.** Each newly added layer image gets its own random
rotation (`setRotation(Phaser.Math.FloatBetween(0, Math.PI * 2))`) at
creation time — applied once per layer, not re-randomized on existing
layers, so earlier layers stay put as later ones stack on top at their own
random angle.

**4. Keep "maximum spray" meaning "fully built up."**
`DIRT_TYPES["bird-poop"].requiredSprayStage` 4 → 8, tracking the new max so
bird-poop still requires the complete stack, not half of it.
`hand-prints.requiredSprayStage` (1, "any spray") is unaffected.

**5. Remove the now-orphaned old assets.** Since the ask is "use them
instead of the spray_patterns," the old 4
`assets/images/spray-patterns/spray_pattern_*.svg` files (including Revision
44's opacity boost) become unreferenced — deleted as part of this change.

**No new pure logic** — `computeSprayStage()` itself needs no changes (it's
already generic over `maxStage`); this is a config/asset swap plus a
scene-side "replace" → "accumulate" rewrite. Verified via headless Chrome:
holding spray on a spot creates progressively more layer images (not one
being destroyed and replaced) with distinct random rotations, all at the
same position/size; `requiredSprayStage` gating for hand-prints/bird-poop
still works at the new scale; decal/layer cleanup on erase or spot-clear
removes every layer, not just one.

Total build-up time roughly doubles (8 layers × 200ms = 1.6s instead of
0.8s) since the existing per-layer duration is kept as-is.

## Revision 48 progress

Implemented exactly as planned, no deviations — 68 tests still passing, no
lint issues, one commit.

- [x] `SPRAY_PATTERNS` repointed to the 8 `build-up/spray_layer_N.svg`
      files with new `spray-layer-N` texture keys; `BootScene` loads from
      the new subpath; `SPRAY_MAX_STAGE` 4 → 8.
- [x] `findOrCreateSprayDecal()`'s decal shape changed `image: null` →
      `layerImages: []`. `setSprayDecalStage()` no longer
      destroys-and-replaces — it loops `decal.stage + 1..stage` calling a
      new `addSprayLayer(decal, layer)` per missing layer (robust against
      ever advancing by more than one in a single call), which creates and
      **keeps** an image at the decal's position/size, giving it its own
      `Phaser.Math.FloatBetween(0, Math.PI * 2)` rotation, and pushes it
      onto `layerImages`.
- [x] `eraseSprayDecalsNear()` now destroys every image in `layerImages`
      (`.forEach()`) instead of a single `.image`; `clearDirtSpot()` was
      already delegating to it, so no separate change needed there.
- [x] `DIRT_TYPES["bird-poop"].requiredSprayStage` 4 → 8, preserving
      "maximum spray" as "the complete stack."
- [x] Deleted the 4 old `spray_pattern_*.svg` files (now unreferenced).

**Verified via headless Chrome:** holding spray for ~1000ms produced a
decal at `stage: 5` with exactly 5 `layerImages` — all at the *same*
position (1 distinct position across all layers) but 5 *distinct* rotations
(confirming per-layer randomization, not a shared/repeated angle);
continuing to hold reached `stage: 8` with 8 layers, matching the new max.
`sprayStageAt()` against bird-poop's spot correctly read `4` after a partial
spray (hold-to-clean refused) and `8` once fully sprayed. A squeegee drag
through a fully-built (8-layer) decal positioned away from any dirt spot
removed it entirely (`sprayDecals.length` 1 → 0) — confirming all 8 layers
were destroyed together, not just one. (One early check of this last case
initially looked like a failure, but turned out to be Revision 43's own
decal-guard correctly protecting a spot the test position had accidentally
landed near — not a regression, and resolved by picking a position
confirmed far from every spot.) No console errors.

No units remaining from this session's agreed scope.

# Revision 49
 - Lets add some stubborn stickers! They are now in ./lost_assets
 - We must move them to a better place
 - To remove the stickers you use the squeege and then you rub on the sticker a lot.
 - We should not press and hold but instead swipe a lot on it.
 - The progress on this is not a progress bar but it changes what image is displayed

## Revision 49 plan

Continues on `main`.

Investigated the new assets: `lost_assets/sticker_peel_stage_1.svg`…`_4.svg`,
each with a clear doc comment describing progressive removal — stage 1
"fully intact," stage 2 "about half peeled," stage 3 "small torn corner,
rest is residue," stage 4 "fully removed, only sticky residue left." Native
size 150×165 (non-square, same treatment as the existing dirt sprites
already get).

**1. Move the assets.** Into `assets/images/dirt/stickers/` — a dedicated
subdirectory, matching the precedent set by `lift/`, `clouds/`, `bubbles/`,
and `spray-patterns/build-up/` for every other multi-file asset set
(bird-poop/hand-prints are single files, so they stay flat in
`assets/images/dirt/`).

**2. New "sticker" dirt type** (`interactionType: "wipe"`, matching
hand-prints' drag-to-clean shape, not bird-poop's hold): `toolIds:
["squeegee"]` only (not sponge), `requiredSprayStage: 0` (no spray needed at
all — reuses the existing `sprayStageAt(...) < requiredSprayStage` gate
as-is, since `0 < 0` is always false, no new branching needed),
`hitsToClean: 16` (starting guess). Added to `brick-town.dirtTypeIds`.

**3. "Changes what image is displayed" instead of a progress bar.** New
`stages` array on the sticker's config entry (4 `{ textureKey, file }`
entries, one per peel stage) — a different shape from bird-poop/hand-prints'
single `textureKey`, so `buildDirtSpots()` generalizes its "does this spawn
as a spot" check and initial-texture pick to
`dirtType.textureKey ?? dirtType.stages[0].textureKey` (and `BootScene`
loads every entry in `stages` the same way it loads a single `textureKey`).

**4. New pure function `computeStickerStage({ hitsRemaining, hitsToClean,
stageCount })`** (`src/utils/`, strict TDD, mirroring `computeSprayStage`'s
own shape/precedent) — divides the hit count into `stageCount` even buckets
and returns which one you're in (1 at the start, clamped at `stageCount`
once fully clean). `wipeDirtSpotsNear()` calls a new
`updateWipeStageImage(spot, dirtType)` right after each registered tap — a
no-op for hand-prints (no `stages` declared), but for stickers swaps
`spot.image`'s texture to the newly-computed stage's key.

**No new pure logic beyond `computeStickerStage`** — everything else is the
same config-driven scene-glue pattern as bird-poop/hand-prints. Verified via
headless Chrome: dragging squeegee repeatedly across a sticker spot advances
it through all 4 images in order (not skipping, not going backward), sponge
alone does nothing to it (tool-restricted), no spray is required first, and
it fully clears (and gates floor completion like every other dirt spot) once
fully rubbed.

## Revision 49 progress

Implemented exactly as planned, no deviations — 71 tests passing (68 before
this revision + 3 new for `computeStickerStage`), no lint issues, two
commits (the pure-logic unit, then the config/scene/asset feature).

- [x] Moved `lost_assets/sticker_peel_stage_1..4.svg` into
      `assets/images/dirt/stickers/`.
- [x] `computeStickerStage({ hitsRemaining, hitsToClean, stageCount })` —
      new pure function in `src/utils/`, grown via strict TDD (3 tests:
      first stage at zero progress, advances once enough hits land, clamps
      at the final stage once fully clean) — same shape/precedent as
      `computeSprayStage`.
- [x] New `sticker` `DIRT_TYPES` entry: `interactionType: "wipe"`,
      `toolIds: ["squeegee"]`, `requiredSprayStage: 0`, `hitsToClean: 16`,
      and a `stages` array of 4 `{ textureKey, file }` entries instead of a
      single `textureKey`. Added to `brick-town.dirtTypeIds`.
- [x] `buildDirtSpots()`'s "does this spawn as a spot" filter and initial-
      texture pick generalized to `dirtType.textureKey ?? dirtType.stages`
      / `dirtType.textureKey ?? dirtType.stages[0].textureKey`.
      `BootScene` gained a second loader loop for every dirt type's
      `stages` array, alongside the existing single-`textureKey` one.
- [x] `wipeDirtSpotsNear()` calls a new `updateWipeStageImage(spot,
      dirtType)` right after each registered tap — a no-op when the dirt
      type declares no `stages` (hand-prints, unaffected), but for stickers
      recomputes the stage via `computeStickerStage()` and swaps
      `spot.image`'s texture to match.

**Verified via headless Chrome:** the sticker spot spawns with texture
`sticker-stage-1`; repeated sponge swipes leave it completely unchanged
(tool-restricted to squeegee, confirmed via `hitsRemaining` staying at 16);
repeated squeegee swipes drove `hitsRemaining` 16 → 11 → 6 → 1 → 0 across 4
separate drags, with `spot.cleared` flipping `true` exactly on the drag that
reached 0 — no spray decal was ever placed near it, confirming the
`requiredSprayStage: 0` gate imposes no precondition. (One early check
misread a destroyed Phaser image's stale `.texture.key` as "stuck at stage
4 forever" — re-verified against `cleared`/`hitsRemaining` directly, which
confirmed the mechanic was correct all along; not a real bug.) No console
errors.

No units remaining from this session's agreed scope.

# Revision 50
 - Lets fix some feedback!
 - Implement the sticker thing above
 - Move the tools out of the bucket. Encapsle the tools with like a square and outline. The selected tool will have different color on the outline to show which one is in use
 - Keep the bucket as sovenir.
 - The thumb is covering the tools and effects. Specially when you spray and remove the bird poop
 - When unscrewing the screw place the scredriver on the screw below so that we can see that it's unscrewing
 - When you're spraying I want the spray bottle to appear from the sides att different random angles pointing towards the spray center to show that we're actually spraying
 - The time it takes to remove brid poop must be randomized 2-3 seconds
 - Zoom in on the level a bit more. Keep this value something I can manually change later to test
 - The tape looks too much lik a roadblock/woodenboard

## Revision 50 plan

Continues on `main`.

**0. Sticker mechanic.** Already implemented in Revision 49 (`computeStickerStage`,
the `sticker` `DIRT_TYPES` entry, asset move, scene glue) — no new work here.

**1. Move the tools out of the bucket.** The bucket's `pointerdown` handler
currently opens a modal tool-selector panel (`openToolSelector`/
`closeToolSelector`, backdrop + panel, `TOOL_SELECTOR_*` constants). That
whole modal is retired: the bucket drops its listener and becomes purely
decorative ("souvenir"), and every tool is shown at all times as a permanent
horizontal row of icons positioned outside the window's bounds, tapping one
calls `selectTool` directly. The `!this.toolSelectorObjects` guards in
`canInteractWithWindow`, `updateToolIcon`, and the pointer handlers are
removed along with the modal.

**2. Square + outline, selection color.** `createToolIcon()` currently
ignores its own `withBorder` flag for every tool with an `iconTextureKey` —
which is all 5 tools — so no tool icon has ever actually shown a border.
Rewritten to always return a small container: a background square with a
stroke, plus the tool's icon image on top. Unselected tools get a dim/neutral
outline; the selected tool gets a distinct highlight color (reusing the
existing green `0x4caf50` progress-ring accent) and a thicker stroke. The
new tool row replaces the old single top-right toolbelt icon+label as the
one place equip state is shown.

**3. Thumb covering tools/effects.** The drag cursor icon
(`updateToolIcon`) and the bird-poop hold effects (progress ring, bubble
spawn) are currently rendered at the exact touch coordinate, i.e. directly
under the finger. A shared upward visual offset is introduced for the
rendered tool icon and for the poop progress ring / bubble spawn position;
all hit-testing keeps using the real, un-offset pointer position. The
screwdriver-on-screw case (item 5) is a deliberate exception to this offset.

**4. Spray bottle "from the sides."** Spraying currently has no source
effect at all — the tool icon is explicitly hidden for spray-bottle, and the
decal itself builds up right where the thumb is. Each time a new spray layer
lands (`addSprayLayer`), a short-lived `spray-bottle` image spawns at a
random angle around the target, positioned out at a radius ("from the
sides"), rotated to point inward at the center, then fades/is destroyed
quickly. This doubles as the fix for "thumb covers effects while spraying,"
since the effect renders away from the touch point by construction.

**5. Screwdriver on the screw.** `handleBoardScrewdriverMove` currently
leaves the drag icon wherever the raw pointer is, rendered at depth 1000 —
above everything — so it can fully cover the screw while unscrewing. While a
screw is actively engaged, the icon snaps to that screw's exact position and
renders behind the screw's own image, so the spinning screw head stays
visible on top. Exact depth values (likely raising the active screw's
container depth above the tool icon's 1000 while engaged) will be tuned
visually via headless Chrome rather than guessed up front.

**6. Randomized bird-poop hold duration.** `dirtTypes.js`'s
`holdDurationMs: 3000` (fixed) becomes `holdDurationRange: [2000, 3000]`,
randomized once per spot at creation time in `createDirtSpotProgress`.
`HoldProgress` needs no change — it already takes a per-instance
`targetDurationMs`.

**7. Zoom in on the level.** No camera/zoom code exists yet. A `LEVEL_ZOOM`
constant near the top of `HouseCleanScene.js`, applied via
`this.cameras.main.setZoom(LEVEL_ZOOM)` in `create()`, makes it a single
manually-tunable value as asked. Every interaction handler currently reads
`pointer.x`/`pointer.y` directly as world coordinates — true today only
because the camera has never moved or zoomed. Once zoomed, these switch to
`pointer.worldX`/`pointer.worldY` throughout (dirt-spot hit-testing,
screw/tape math, `isInsideWindow`) so touch input still lines up with what's
rendered. Verified via headless Chrome across every interaction type
post-zoom.

**8. Tape re-style.** `police_tape_middle.svg`/`police_tape_edge.svg` have a
thick near-black 5px frame band and fully opaque stripes, reading as a solid
wooden plank rather than thin plastic tape. No new art was supplied for this
one (unlike stickers/spray-layers in past revisions), so the existing SVGs
are restyled directly: drop/thin the dark frame band and reduce overall
opacity so it reads as flimsy tape, keeping the diagonal stripe pattern
intact. A judgment call in the absence of a reference asset — open to
iterating after seeing it in-game.

## Revision 50 progress

Implemented all 8 remaining items (the sticker mechanic was already done via
Revision 49) — no new pure-logic units this session, same shape as Revision
48: config/scene rewiring verified via a scripted headless-Chromium session
rather than vitest, since `HouseCleanScene` itself has no unit tests. 71
tests still passing throughout, no lint issues, eight commits (one per
item, plus a follow-up fix discovered during verification).

- [x] Bucket retired to decoration; tools now live in a permanent row
      (`createToolRowIcon`) instead of behind a bucket-tap modal
      (`openToolSelector`/`closeToolSelector` removed along with the
      `TOOL_SELECTOR_*` constants).
- [x] `createToolIcon` rewritten — every tool row icon gets a square
      background + outline; the selected tool's outline is green
      (`0x4caf50`, thicker) vs. a dim white default. This also fixed a
      pre-existing dead `withBorder` parameter that never actually worked
      for any of the 5 (all image-icon) tools.
- [x] `TOUCH_EFFECT_OFFSET_Y` (-90) applied to the drag tool icon and the
      bird-poop hold ring/bubbles, so touch effects render above the
      pointer instead of directly under the thumb.
- [x] `spawnSprayBottleEffect` — a spray-bottle image flashes in from a
      random angle around the spray target each time a new layer lands,
      aimed inward; the only visual cue that spraying is happening
      (doubles as the fix for "thumb covers effects while spraying").
- [x] Screwdriver anchors on the actively-engaged screw
      (`screwdriverImage`, added before the screw image in the container
      so it renders behind it) instead of following the free-floating
      cursor; the generic tool icon hides while engaged.
- [x] bird-poop's `holdDurationMs` (fixed 3000) replaced with
      `holdDurationRange: [2000, 3000]`, randomized once per spot in
      `createDirtSpotProgress`.
- [x] `LEVEL_ZOOM` (1.15) applied via `camera.setZoom`; every interaction
      handler's `pointer.x`/`pointer.y` switched to
      `pointer.worldX`/`worldY` (or a manual `getWorldPoint` conversion for
      `prevPosition`) so hit-testing still lines up post-zoom.
      `rotateToolTowardSweep`'s delta deliberately stays screen-space
      (direction-only, zoom-invariant).
- [x] Fixed a zoom side effect caught during verification: the single
      zoomed camera was also zooming HUD elements near the canvas edges
      (pause/skip buttons, floor label) out of view. Fixed by parenting
      every gameplay object under a new `worldContainer` (rendered only by
      the zoomed main camera) and all HUD under a new `hudContainer`
      (rendered only by a new unzoomed `uiCamera`); each camera ignores
      the other's container.
- [x] `police_tape_middle.svg`/`police_tape_edge.svg` restyled — thin edge
      lines and reduced opacity instead of a thick dark frame band, so it
      reads as plastic tape instead of a wooden board/roadblock.

**Verified via a scripted headless-Chromium session** (puppeteer-core
driving the real static build, not a mock): the tool row renders with the
correct square+outline styling and swaps the selected highlight on click;
a squeegee drag across the window at zoom 1.15 correctly advanced
`revealedFraction` (0 → 0.51), confirming touch coordinates still map to
world objects post-zoom; spraying a spot reached stage 8 and the
bottle-burst effect was visible on screen; holding sponge on the sprayed
bird-poop spot cleared it, with the ring/bubbles rendered above the touch
point — visibly outside the window in one screenshot, so the offset may be
a little generous at this zoom level and worth a look in-game; the tape's
new thin-plastic look and the screwdriver anchoring behind an engaged
screw (`screwdriverVisible: true`, generic cursor hidden) were both
confirmed in an earlier pass of the same session. No console/page errors
beyond a harmless missing-favicon 404.

No units remaining from this session's agreed scope.

# Revision 51
 - Look at this: ![alt text](image.png)
      - The z-index of the lift etc is too low. It should be greater than the windows + obstructions but lower than the tools etc.
 - When using the tools the tools should appear exactly where you click. Make them bigger instead.
 - Remove the screwdriver when unscrewing the screw
 - The sticker needs more rubbing before it dissapears.
 - The spray bottle flashes to much. Every time a new spray pattern appear the spray bottle should appear.

## Revision 51 plan

Continues on `main`.

**1. Lift z-index.** Live-tested rather than guessed: forcing a lift/window
overlap in a headless browser and sampling the rendered pixel showed the
board obstruction (depth 0) rendering in front of the lift platform (depth
10) — backwards from what the depth values say. Root cause: Revision 50's
HUD/zoom fix moved every gameplay object into a `worldContainer` so it could
be zoomed independently of the HUD, but `Phaser.GameObjects.Container` does
not auto-sort children by `.depth` the way the Scene's own display list
does — it renders in insertion order, and `liftContainer` is created before
any floor segments. A regression from that fix, not something in the
original design. **Fix:** call `this.worldContainer.sort("depth")` at the
end of `create()` and again inside `spawnFloorSegment`, `spawnClouds`, and
`refreshDraggingToolIcon` (every point new children are added to it
afterward). No depth *values* need to change — the existing ordering (sky
-30 < skyline -20 < clouds -10 < segments 0 < lift 10 < drag tool icon
1000) already matches "lift above windows/obstructions, below tools," it
just needs sorting to actually take effect.

**2. Tools appear exactly where you click, made bigger instead.** Replaces
Revision 50's `TOUCH_EFFECT_OFFSET_Y` (-90), which rendered too far from the
touch point. The drag tool icon and the bird-poop hold ring/bubbles go back
to the pointer's exact position (`pointer.worldX`/`worldY`, no offset).
`DRAGGING_TOOL_ICON_SIZE` grows 100 → 160 to compensate; since
`POOP_PROGRESS_RADIUS` is already defined as `= DRAGGING_TOOL_ICON_SIZE`,
the hold ring grows proportionally for free. `TOUCH_EFFECT_OFFSET_Y` is
removed entirely.

**3. Remove the screwdriver while unscrewing.** Reverts the other half of
Revision 50's screwdriver-on-screw change: `screwdriverImage` and
`SCREWDRIVER_ON_SCREW_SIZE` are dropped from `buildScrew` entirely. The
generic tool icon still hides while a screw is actively engaged (as it does
now) — nothing replaces it. The spinning screw itself is the only visual
feedback, guaranteeing nothing can obscure it.

**4. Sticker needs more rubbing.** `sticker.hitsToClean` in
`dirtTypes.js`: 16 → 24 — a starting guess, easy to retune.

**5. Spray bottle flashes less.** The trigger ("every time a new spray
pattern/layer appears") is already correct as described — no change there.
"Flashes too much" reads as the character of each individual flash:
currently a sharp 90ms-in/90ms-out linear blink repeating every 200ms while
holding, which strobes. `SPRAY_BOTTLE_EFFECT_DURATION` 180 → ~360 with the
tween switched to `Sine.easeInOut` so each appearance reads as a smooth
flash-and-fade instead of a blink — a judgment call to verify visually and
retune if it still feels busy.

`image.png` (the annotated screenshot) is committed alongside this spec
update since the spec embeds it by relative path.

## Revision 51 progress

Implemented all 5 items — no new pure-logic units this session, same shape
as Revisions 48/50: scene/config changes verified via a scripted
headless-Chromium session rather than vitest. 71 tests still passing
throughout, no lint issues, five commits.

- [x] `worldContainer.sort("depth")` called at the end of
      `spawnFloorSegment`, `spawnClouds`, and `refreshDraggingToolIcon` —
      confirmed fixed by re-running the exact live pixel-overlap test that
      diagnosed the bug: the lift platform now renders in front of the
      board obstruction instead of behind it.
- [x] `TOUCH_EFFECT_OFFSET_Y` removed entirely; the drag tool icon and the
      bird-poop hold ring/bubbles render at the exact pointer position
      again. `DRAGGING_TOOL_ICON_SIZE` 100 → 160 (and `POOP_PROGRESS_RADIUS`
      grows with it, since it's defined from the same constant).
- [x] `screwdriverImage`/`SCREWDRIVER_ON_SCREW_SIZE` removed from
      `buildScrew` entirely — confirmed via live state that screws no
      longer carry a `screwdriverImage` property at all. The generic tool
      icon still hides while a screw is engaged (unchanged); nothing
      replaces it.
- [x] `sticker.hitsToClean` in `dirtTypes.js`: 16 → 24.
- [x] `SPRAY_BOTTLE_EFFECT_DURATION` 180 → 360 with `Sine.easeInOut` on the
      alpha tween, so each spray-bottle flash fades smoothly instead of
      blinking.

**Verified via a scripted headless-Chromium session**: the tool icon
rendered within half a world-unit of the exact intended click position
(displaySize 160, confirming both the offset removal and the size bump);
spraying still reached stage 8 with the new tween; the sticker's
`hitsToClean` read back as 24 from the live config; and on a floor with a
board obstruction, engaging a screw showed `toolIconVisible: false` and
`hasScrewdriverImage: false` on both screws, confirming the screwdriver
is fully absent while unscrewing. No console/page errors beyond the
harmless missing-favicon 404.

No units remaining from this session's agreed scope.

# Revision 52
 - Make the tool selections bigger ![alt text](image-1.png)
 - Make the sticker twice as stubborn to remove

## Revision 52 plan

Continues on `main`.

**1. Bigger tool selection icons.** `TOOL_ROW_ICON_SIZE` (60 → 80) and
`TOOL_ROW_GLYPH_SIZE` scaled proportionally (44 → 60).
`TOOL_ROW_LABEL_OFFSET_Y` is already derived from `TOOL_ROW_ICON_SIZE`, so
it grows automatically. Row width goes from 396px to 496px (5 icons + 4
gaps), comfortably fitting even the narrowest supported canvas width
(~720px) — no layout changes needed. Label font size 11px → 12px to stay
proportionate to the larger icons.

**2. Sticker twice as stubborn.** `sticker.hitsToClean` in `dirtTypes.js`:
24 → 48 (doubling the value Revision 51 just set).

`image-1.png` (the annotated screenshot) is committed alongside this spec
update since the spec embeds it by relative path.

## Revision 52 progress

Implemented both items — pure constant tweaks, no new logic. 71 tests
still passing, no lint issues, two commits.

- [x] `TOOL_ROW_ICON_SIZE` 60 → 80, `TOOL_ROW_GLYPH_SIZE` 44 → 60, label
      font size 11px → 12px. Verified via headless Chrome at both the
      normal (936px) and narrowest supported (720px) canvas widths — the
      row fits comfortably with margin at both.
- [x] `sticker.hitsToClean` in `dirtTypes.js`: 24 → 48.

No units remaining from this session's agreed scope.

# Revision 53
 - Make the sticker need 3 times more swiping before removed

## Revision 53 plan

Continues on `main`.

**Sticker needs 3x more swiping.** `sticker.hitsToClean` in `dirtTypes.js`:
48 → 144 (3x the current value, consistent with how "twice as stubborn"
was interpreted last revision as ×2 of the then-current value).

## Revision 53 progress

- [x] `sticker.hitsToClean` in `dirtTypes.js`: 48 → 144. 71 tests still
      passing, no lint issues, one commit.

No units remaining from this session's agreed scope.

# Revisino 54
 - This is what the main menu look like: ![alt text](image-2.png)
 - This is what a level looks like: ![alt text](image-3.png)
 - Change the size of the skip button (it's for testing only)
 - Give the main menu a better screen
      - We do not need the toolbelt or Lift Garage
            - Maybe we do need it but that could be in a separate tab settings or something.
 - I'm giving you 100% freedom. If we're missing some assets you go ahead and create it.

## Revision 54 plan

Continues on `main`.

**1. Smaller skip button.** It's a debug-only affordance
(`buildDebugSkipButton` in `HouseCleanScene.js`), currently sized the same
as the pause button (80×36, 14px label) — too prominent for something not
meant for real players. Shrunk to ~50×22 with a 9px label, same position.

**2. Main menu redesign.** Current screen (`MainMenuScene.js`): plain black
background, 4 flat-color house cards, then a purely decorative "Toolbelt"
row (no click handlers — dead weight now that Revision 50 moved
tool-switching in-game to the tool row) and a "Lift Garage" picker, stacked
all the way down the screen.

- **Remove the Toolbelt section entirely** — `createIconRow` and the now-
  unused `TOOLS` import go with it. Purely decorative, duplicates the
  in-game tool row.
- **Move Lift Garage into a "Settings" modal** — a button below the house
  grid opens a backdrop + panel (mirroring the tool-selector modal pattern
  already used elsewhere in this codebase) containing the existing
  lift-picker UI, logic untouched, just relocated. Keeps the real
  lift-selection feature (Gondola/Hanging Board actually affect gameplay)
  without cluttering the main screen.
- **Atmospheric background** — a vertical dusk-gradient fill (Phaser
  `fillGradientStyle`, no new asset needed) instead of flat black, so the
  screen feels designed rather than empty. House-agnostic, so it won't
  look mismatched once other houses unlock later.
- **Nicer house cards** — rounded corners (`Graphics.fillRoundedRect`)
  instead of sharp rectangles; for Brick Town (the one house with real
  art), a cropped thumbnail of its own skyline art (`brick-town-skyline`,
  already loaded) behind the title/info text with a dark gradient overlay
  for legibility — like a level-select tile. "Coming soon" cards stay
  flat/muted placeholders with the same rounded treatment for consistency.
- With the Toolbelt/Lift Garage gone, the freed vertical space goes to
  better centering/margins rather than making the cards huge.

No new SVG assets needed — reusing existing in-game art keeps the menu
visually consistent with gameplay. Exact visual polish (gradient colors,
corner radius, spacing) is a judgment call, tuned and verified via
screenshots during implementation, same as prior aesthetic calls (tape
restyle, spray timing).

`image-2.png`/`image-3.png` (the annotated screenshots) are committed
alongside this spec update since the spec embeds them by relative path.

## Revision 54 progress

Implemented both items — no new pure-logic units, scene-only changes
verified via a scripted headless-Chromium session. 71 tests still
passing, no lint issues, five commits (skip button, toolbelt removal,
gradient background, rounded/art-backed cards, settings modal).

- [x] Debug skip button shrunk 80×36 → 50×22, label 14px → 9px.
- [x] Toolbelt section removed entirely (`createIconRow` and the `TOOLS`
      import deleted — it had no click handlers and duplicated the
      in-game tool row).
- [x] Gradient background — **hit a real Phaser gotcha**:
      `Graphics.fillGradientStyle` renders as flat black under Phaser's
      Canvas renderer (this game runs Canvas, not WebGL), so the gradient
      never appeared no matter the colors used. Worked around by drawing
      the gradient onto a real 2D canvas context once in `BootScene`
      (`this.textures.createCanvas` + `ctx.createLinearGradient`) and
      displaying it as a plain image in `MainMenuScene` — renderer-
      agnostic, confirmed visually.
- [x] House cards rounded (`Graphics.fillRoundedRect`/`strokeRoundedRect`)
      with a plain transparent `Rectangle` overlaid for click handling
      (Graphics objects aren't natively interactive). Brick Town gets a
      thumbnail of its own skyline art (`brick-town-skyline`) — **also hit
      a gotcha**: `Image.setDisplaySize()` after `setCrop()` scales
      against the *full* uncropped texture size, not the crop's, so the
      cropped content rendered far larger than intended and overflowed
      into neighboring cards. Switched to scaling the full image to the
      thumbnail's width, bottom-anchoring it (so the buildings near the
      bottom of the tall source art show, not empty sky), and clipping to
      the thumbnail box with a `GeometryMask` — self-correcting regardless
      of position math, unlike crop+scale. Title/status text moved to the
      bottom of every card (previously upper-middle) over a dark backdrop
      band, matching a level-select tile look.
- [x] Lift Garage moved into a `Settings` modal (backdrop + rounded panel,
      mirroring the tool-selector modal pattern previously used in
      `HouseCleanScene`) opened via a new button below the house grid;
      the existing lift-selection logic (`selectLift`,
      `liftSelectionBorders`) is unchanged, just rebuilt fresh into the
      modal's own object list each time it opens and destroyed on close.

**Verified via a scripted headless-Chromium session**: house-card
navigation to `HouseCleanScene` still works after switching its hit
target to a plain rectangle; the settings modal opens, lift selection
correctly moves the selection border and updates the registry
(`gondola` → `hanging-board`), and closes on an outside tap; layout holds
up with no overflow at both the normal (936px) and narrowest supported
(720px) canvas widths. No console/page errors beyond the harmless
missing-favicon 404.

No units remaining from this session's agreed scope.

# Revision 55
 - Create a level which is has infinite floors. Give this level a fitting name.
 - On this level the dirt and obstructions occur randomly but every floor has atleast one of them. Multiple of the same things can appear on the same floor

## Revision 55 plan

Continues on `main`. This is the largest revision so far — a longer
implementation session than usual, structured inside-out across 5 phases.

**Phase 1 — generalize obstructions to a list (pure refactor, no behavior
change for existing houses).** Currently `segment.obstruction` is singular,
and board/screw/tape math is hardcoded against `WINDOW_Y` as if there's
exactly one obstruction dead-centered in the window. Generalizing:
- `segment.obstructions` (array) + `this.activeObstructions`. Each
  obstruction gets its own `bandOffsetY` (0 for the single-obstruction
  case, so existing houses are byte-for-byte unaffected).
- Board's plank + both screws move into a shared, non-rotating
  `obstructionGroup` container positioned at `(0, bandOffsetY)` — the
  plank's own hinge/pivot/fall math is already relative to that group's
  local origin, so it's unaffected by the offset. Tape's existing
  `tapeContainer` just gets repositioned to `bandOffsetY` directly.
- `canInteractWithWindow`, `handlePointerDown`/`up`, `handlePointerMove`,
  `handleObstructionPointerMove` generalize to operate over the array —
  `handleObstructionPointerMove` now picks whichever obstruction matches
  the equipped tool's type *and* is nearest the pointer, instead of
  assuming one.
- `handleBoardScrewdriverMove`/`handleTapeSwipeMove`/`completeScrew`/
  `animateBoardFall`/`completeTapeCut` take the specific obstruction as a
  parameter instead of reading `this.activeObstruction` implicitly, and
  clear by removing from the array instead of nulling a single reference.
- `updateRevealProgress`'s floor-complete check becomes
  `activeObstructions.length === 0`.

Verified via the existing test suite plus live regression testing on
Brick Town (still exactly one obstruction per floor there) — screwdriver,
scissors, and the block-everything-else-while-an-obstruction-exists
behavior should be pixel-for-pixel unchanged.

**Phase 2 — the new house.** Adds to `houses.js`: id `endless-ascent`, name
**"Endless Ascent"**, `infinite: true`, no `floors` value. Reuses Brick
Town's wall/ground/skyline art for now (no new assets requested this
round) — visually distinguished later if wanted.

**Phase 3 — procedural per-floor content.** For `house.infinite` only,
`spawnFloorSegment` replaces the fixed "one of each dirt type, one random
obstruction" logic with: roll an obstruction count (weighted 0–2, capped
at 2 given window height and screw hit-radius constraints), roll a
dirt-spot count (0–3), each slot independently randomized with repetition
allowed (`toolIds`-compatible types only) — if both counts land on 0,
force one dirt spot so every floor has at least one thing. Two
simultaneous obstructions get `bandOffsetY` of about ∓130 (tuned visually
during implementation).

**Phase 4 — infinite floor count.** Every place that assumes a finite
`house.floors` branches on `house.infinite`: `roofLineY()` (never places a
roof), `skylineParallaxTravel()` (caps travel at a fixed distance instead
of `(floors-1)*spacing`), `formatFloorLabel` (new "Floor N" form with no
total — a real unit, TDD'd with a new test case), `advanceFloor()`'s
completion check (never fires), `ensureFloorSpawned`/`debugSkipFloor`'s
upper-bound checks (always pass).

**Phase 5 — menu integration.** `MainMenuScene.js` gets a 5th card. Rather
than awkwardly filling the 2×2 grid's leftover slot, it renders as a
distinct full-width tile below the grid (visually marking it as a
different mode, not just another building), with the Settings button
shifting down to make room.

## Revision 55 progress

Implemented all 5 phases exactly as planned. 76 tests passing (2 new
units: `planInfiniteFloorContent`, `formatFloorLabel`'s no-total case), no
lint issues, seven commits (one per phase, plus a follow-up commit for
the floor-spawning bound that Phase 4's own generalization needed to
actually fix).

- [x] **Phase 1**: `segment.obstruction` → `segment.obstructions`
      (array), each with its own `bandOffsetY` baked into its children's
      authored positions at construction time (no extra wrapper
      container needed — screws/tape band math already worked out to be
      offset-invariant once the offset was baked in consistently).
      `handleObstructionPointerMove` now filters by type and hands the
      matching list to `handleBoardScrewdriverMove`/`handleTapeSwipeMove`,
      which iterate every obstruction of that type (screws already
      self-gate by hit-radius; tape gets a new `TAPE_ACTIVATION_MARGIN`
      Y-bound that only activates once there's more than one tape, so
      the single-tape case is provably unchanged). Verified via a full
      circular-drag/tape-cut regression script driving 5 real floors on
      Brick Town to completion — no regressions.
- [x] **Phase 2**: `endless-ascent` / "Endless Ascent" added to
      `houses.js`, reusing Brick Town's art.
- [x] **Phase 3**: `planInfiniteFloorContent` (new pure unit, TDD'd,
      injectable `random` like `cloudSpec.js`) rolls a weighted 0–2
      obstruction count and a 0–3 dirt count independently, each slot
      randomized with repeats allowed, forcing one dirt spot if both
      would land on zero. Wired into `spawnFloorSegment` via a new
      `planFloorContent()` that keeps curated houses on their exact
      original fixed-content path.
- [x] **Phase 4**: `skylineParallaxTravel` caps at a fixed distance for
      `house.infinite`; `formatFloorLabel` gained a no-total form (new
      unit test); `ensureFloorSpawned`'s bound was actually backwards for
      `undefined` floors (`floor <= undefined` is always false — floor 2
      would never have spawned) — fixed alongside making
      `advanceFloor`/`debugSkipFloor`'s bounds explicit via a shared
      `isPastLastFloor` helper instead of relying on `undefined`
      comparison quirks that happened to work by accident.
- [x] **Phase 5**: `MainMenuScene.createHouseCard`/thumbnail/mask logic
      extracted into a shared `drawHouseTile`, reused by a new
      `createInfiniteHouseCard` for the full-width tile.

**Verified via a scripted headless-Chromium session** driving the actual
menu → infinite house → 15 real floors of skip-through play: every floor
had at least one thing (never both empty), floor 9 produced a
board+police-tape mix and floor 10 produced two simultaneous boards in
one run, and a separate run's floor 14 (screenshotted) showed two
police-tape strips cleanly spaced with no overlap — confirming Phases 1
and 3 compose correctly end-to-end, not just in isolation. No console/page
errors beyond the harmless missing-favicon 404.
